...
### Round 12: LLM 工具选择错误 + 破坏性计划覆盖

**日期**: 2026-02-23
**环境**: macOS + Docker + devcontainer，模型同前

#### Issue #22: LLM 错误调用 `generate_learning_plan` 覆盖已有学习计划

**严重程度**: HIGH
**修复状态**: 已修复

**复现步骤**:
1. 先发送"我现在的学习计划是什么？" → Agent 调用 `get_learning_plan`（2 次），正常返回 4 个学习计划，其中 Tidy First 有 **33 个概念**
2. 再发送"告诉我 tidy first 的学习计划"

**期望**: 调用 `get_learning_plan(topic_name="Tidy First")` 一次，返回详细计划

**实际**:
- `get_learning_plan` 被调用 **3 次**（LLM 对返回格式不满意重试）
- `list_collections` 被调用 1 次
- `generate_learning_plan` 被调用 **2 次**（LLM 疑似传了 `force=true`）
- 最终结果: Tidy First 计划被覆盖为 **9 章/24 小节**，原来的 33 个概念丢失

**根因分析**:
1. **System prompt 缺少工具选择指导**: `Tool Usage Protocol` 只规定了"先调工具再说话"，没有指导 LLM 哪种场景用哪个工具
2. **工具描述重叠**: `get_learning_plan` 和 `generate_learning_plan` 的描述都涉及"学习计划"，LLM 分不清该用哪个
3. **`force=true` 无防护**: LLM 可以自行决定传 `force=true` 覆盖计划，无数据丢失检测

**修复（3 项）**:
1. **System prompt 增加 Tool Selection Guide**（`mentor_system_prompt.md`）— 明确查看→`get_learning_plan`，创建→`generate_learning_plan`；强调 `force=true` 破坏性；禁止同参数重试
2. **工具描述改进**（`app/tools/__init__.py`）— `get_learning_plan` 标注为 PRIMARY；`generate_learning_plan` 标注 ONLY for CREATE；`force` 标为 DANGEROUS
3. **Data-loss guard**（`learning_plan_tool.py`）— 保存前比较概念数：现有 > 新 → 中止并保留旧计划；覆盖单文档和多文档两个路径

**测试**: 295 passed。

#### Issue #23: Streaming tool loop 无原生 thinking 指示器

**严重程度**: LOW
**修复状态**: 已修复

**现象**: 恢复 buffer+discard 策略（double-response 修复）后，tool loop 的 buffer 阶段（LLM 生成中，3-5 秒）不发送任何 SSE 事件，Open WebUI 不显示原生 thinking 指示器。直连 LLM 时无此问题。

**根因**: buffer 循环中所有 chunk 均被缓存，空 delta 和 role-only delta 也未转发，客户端无法感知连接活跃状态。

**修复**（`agent_service.py`）:
- buffer 循环中把非内容 chunk（空 delta、role-only delta）直接转发给客户端
- 内容 chunk 继续缓存（防 double-response），tool_calls 和 finish_reason chunk 不转发
- 与直连 LLM 行为一致：空 delta 触发 Open WebUI 原生 thinking 动画

**测试**: 295 passed。

#### Issue #24: `get_learning_plan` 重复调用（复测发现）

**严重程度**: MEDIUM
**修复状态**: 已修复

**现象**: Issue #22 修复后复测，LLM 不再调用 `generate_learning_plan`，但 `get_learning_plan` 仍被调用 3 次才返回结果。

**根因分析**:
1. **`_find_existing_topic` 只支持精确/大小写匹配**: LLM 传 "Tidy First" 但 DB 中可能是 "Tidy First?"（含标点），精确匹配失败
2. **"Not found" 消息误导**: 返回 "Use generate_learning_plan to create one first" — 与 Tool Selection Guide 矛盾，引导 LLM 走错路
3. **"Not found" 不列出可用名称**: LLM 不知道正确的 `topic_name` 拼写，只能盲目重试

**修复（3 项）**:
1. **`_find_existing_topic` 增加子串匹配**（Pass 3）— 双向子串包含（"Tidy First" ⊂ "Tidy First?"），唯一匹配才返回
2. **"Not found" 消息改进** — 列出所有可用 plan 名称，提示用正确名称重试
3. **去除误导性 hint** — 从 "Use generate_learning_plan" 改为 "Call get_learning_plan with exact topic_name"
...
| 时间 | 事件 | 结论 |
|---|---|---|
| T88 | 恢复 fast path + buffer+discard（double-response 修复） | 295 passed |
| T89 | 复测: tool loop 期间 Open WebUI 无 thinking 指示器 | Issue #23 — 转发非内容 chunk |
| T90 | 复测: "告诉我 tidy first 的学习计划" → 6 次工具调用，计划被覆盖（33→24） | Issue #22 — prompt + 描述 + data-loss guard |
| T91 | 实施 Issue #22 三项修复 | 295 passed |
| T92 | 复测: `generate_learning_plan` 不再误调，但 `get_learning_plan` 仍调 3 次 | Issue #24 — 子串匹配 + not-found 消息改进 |
| T93 | 实施 Issue #24 三项修复 | 295 passed |

### Round 12 状态汇总

- Issue #22（LLM 工具选择错误 + 破坏性覆盖）: **已修复**
- Issue #23（Streaming 无 thinking 指示器）: **已修复**
- Issue #24（`get_learning_plan` 重复调用）: **已修复**
- 测试总数: 295（无变化，仅修改了 1 个已有 force 测试的 mock）

### Round 13: Story 1-6 E2E 验证 — 双 Provider 路由

**日期**: 2026-02-24
**环境**: macOS + Docker + devcontainer, 双 Provider（claude-sub-sonnet-4-6 via claude-max-proxy + deepseek-v3-2 via NVIDIA API）

#### Issue #25: `_find_existing_topic` 对同一文档不同作者名格式无法去重 → 重复 topic

**严重程度**: MEDIUM
**发现步骤**: Story 1-6 E2E 验证（双 Provider 路由测试）— 用户查看学习计划时发现同一本书出现两次
**修复状态**: 已修复

**现象**: `get_learning_plan` 返回 6 个计划，其中《A Philosophy of Software Design, 2nd Edition》出现两次：
- id=4: 9 个概念, topic name = `A Philosophy of Software Design, 2nd Edition (Ousterhout, John) (z-library.sk, 1lib.sk, z-lib.sk)`
- id=8: 7 个概念, topic name = `A Philosophy of Software Design, 2nd Edition (John K. Ousterhout) (z-library.sk, 1lib.sk, z-lib.sk)`

两次提取的概念完全不同（id=4 提取了设计原则，id=8 提取了目录结构），说明是两次独立的 LLM 分析。

**根因分析**:

topic name 来自 `_clean_filename(filename)` — 去掉 `.pdf` 后缀即为 topic name。Open WebUI 对同一 PDF 的两次上传/索引产生了不同的作者名格式（`Ousterhout, John` vs `John K. Ousterhout`），导致 filename 不同。

`_find_existing_topic` 的三级匹配全部失败：
1. **精确匹配** — 失败（作者名不同）
2. **大小写不敏感** — 失败（同上）
3. **子串匹配** — 失败（两个完整字符串互不包含，因为括号内容不同导致整串互不包含）

因此第二次 `generate_learning_plan` 认为这是新书，创建了新 topic。

**修复**:
1. `_find_existing_topic` 新增 **Pass 4: 模糊词集匹配** — 将 topic name 拆为单词集合（去标点、小写化、过滤单字符和噪音域名如 `z-library.sk`），用 Jaccard 相似度比较。阈值 0.8。这样 `(Ousterhout, John)` 和 `(John K. Ousterhout)` 产出相同的 token 集合 `{john, ousterhout}`（`K` 为单字符被过滤），相似度 1.0。同时不同书籍之间相似度最高仅 0.14，远低于阈值，不会误匹配。唯一匹配才返回，多匹配返回 None。
2. **数据修复**: 删除重复的 topic id=8 及其 7 个 concepts（无关联 edges）

**测试**: +7 新测试（`TestTokenizeName` 3 个 + `TestNameSimilarity` 4 个），全量 326 passed

**关联文件**: `learning_plan_tool.py`（`_tokenize_name`, `_name_similarity`, `_find_existing_topic` Pass 4）, `tests/unit/test_learning_plan_tool.py`

### Round 13 时间线

| 时间 | 事件 | 结论 |
|---|---|---|
| T94 | Story 1-6 E2E: 配置双 Provider（claude + NVIDIA DeepSeek），重建容器 | 双 provider /v1/models 返回 2 个模型 |
| T95 | DeepSeek 对话: "当前学习计划是什么" → 6 个计划，同一本书出现两次 | Issue #25 — _find_existing_topic 去重不足 |
| T96 | DB 查询确认: topic id=4 (9 concepts, Ousterhout, John) vs id=8 (7 concepts, John K. Ousterhout) | 同一 PDF 不同作者名格式 |
| T97 | 修复: _find_existing_topic 增加核心标题匹配（去括号后缀） | 全量通过 |
| T98 | 数据修复: 删除重复 topic id=8 及关联数据 | DB 清理完成 |

### Round 13 状态汇总

- Issue #25（重复 topic — 作者名格式差异）: **已修复**
