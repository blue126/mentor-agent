...
- 在不影响主流程的前提下，新增 Telegram 作为补充入口（文字输入输出 + 主动复习提醒）
- 保持 Open WebUI 为 v1 主入口，Chatbox 为移动补充入口，三者共享同一 Agent Service

### 架构解耦方向

- 入口层仅处理 I/O，编排层处理业务流程，能力层通过适配器接入
- 保持现阶段实现轻量，避免把全部能力堆进 Agent Service

### RAG 演进策略

- v1/v2 继续使用 Open WebUI 开箱 RAG，优先保证交付速度和维护成本
- 仅在出现权限隔离、检索质量、可观测性等明确需求时，评估自建 RAG（如 Qdrant）
- 目标是“替换适配器，不重写业务编排”
